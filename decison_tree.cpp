...
public:
	...
	void gen(dataset cur_dataset, int node_idx) { dbg(node_idx);
		...
		dbg(cur_dataset.data);
		dbg(node_idx, tree[node_idx].max_entropy_gain_idx);
...
int main() {
	...
	for (int i = 0; i < cur_dataset.data.size(); ++i) {
		dbg(cur_dataset.data[i]);
	}
	decision_tree dt(cur_dataset);
	vector<string>lol = {"Sun[9:00-10:00]","Yes","Home","ON","Not Charging","Vibration","Normal","Games"};
	int yo = dt.dfs(lol, 0), cnt = 0;;

	for (int i = 0; i < cur_dataset.data.size(); ++i) {
		int yo = dt.dfs(cur_dataset.data[i], 0);
		dbg(dt.tree[yo].label);
